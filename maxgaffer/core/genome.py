...
PARAMS: Tuple[ParamSpec, ...] = (
    ...
    # Multi-light roles are role-sorted by the bridge from artist names
    # (Key/Main, Fill, Reflection/Rim, Background). Four of each covers complex archviz
    # rigs without allowing an unbounded model-authored parameter surface.
    *(spec for index in range(2, 5) for spec in (
        ParamSpec(f"sun{index}.enabled", 0, 1, 1,
                  doc=f"role sun {index} on/off"),
        ParamSpec(f"sun{index}.azimuth_deg", 0.0, 360.0, 60.0, wrap=True,
                  doc=f"role sun {index} compass azimuth"),
        ParamSpec(f"sun{index}.altitude_deg", -4.0, 88.0, 25.0,
                  doc=f"role sun {index} elevation"),
        ParamSpec(f"sun{index}.intensity", 0.0, 20.0, 1.0, log_scale=True,
                  doc=f"role sun {index} intensity"),
        ParamSpec(f"sun{index}.size", 0.5, 30.0, 1.0, log_scale=True,
                  doc=f"role sun {index} shadow softness"),
        ParamSpec(f"sun{index}.turbidity", 1.8, 10.0, 1.5,
                  doc=f"role sun {index} atmospheric haze"),
    )),
    *(spec for index in range(2, 5) for spec in (
        ParamSpec(f"dome{index}.enabled", 0, 1, 1,
                  doc=f"role dome {index} on/off"),
        ParamSpec(f"dome{index}.rotation_deg", 0.0, 360.0, 90.0, wrap=True,
                  doc=f"role dome {index} horizontal rotation"),
        ParamSpec(f"dome{index}.intensity", 0.0, 20.0, 1.0, log_scale=True,
                  doc=f"role dome {index} multiplier"),
    )),
    ParamSpec("atmosphere.enabled", 0, 1, 1,
              doc="V-Ray Environment Fog/Aerial Perspective on/off"),
    ParamSpec("atmosphere.distance_m", 0.1, 10000.0, 1.0, log_scale=True,
              doc="physical visibility/fog distance in metres; lower is denser"),
    ParamSpec("atmosphere.height_m", -1000.0, 10000.0, 100.0,
              doc="physical fog height in metres"),
